...
class Event extends Model
{
    ...
    public function scopeFilter($query, array $filters)
    {
        ...
        })->when($filters['status'] ?? null, function ($query, $finished) { // this is for finished filters
            $query->where('status', 'like', '%' . $finished . '%');
        })->when($filters['status'] ?? null, function ($query, $upcomingwith7days) { // filter start with now date to +7 days
        ...
        })->when($filters['status'] ?? null, function ($query, $finishedwith7days) { // with end date upto 7 days
            dd($finishedwith7days);
